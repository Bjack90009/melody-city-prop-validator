(() => {
  ...
  async function applyWorkbook(buffer, sourceName) {
    ...
    const required = ["规则配置", "品质配置", "道具配置"];
    const missing = required.filter((name) => !workbook.Sheets[name]);
    if (missing.length) throw new Error(`缺少工作表：${missing.join("、")}`);

    const ruleRows = XLSX.utils.sheet_to_json(workbook.Sheets["规则配置"], { defval: "" });
    const qualityRows = XLSX.utils.sheet_to_json(workbook.Sheets["品质配置"], { defval: "" });
    const itemRows = XLSX.utils.sheet_to_json(workbook.Sheets["道具配置"], { defval: "" });
    const performanceRuleRows = workbook.Sheets["表演规则"] ? XLSX.utils.sheet_to_json(workbook.Sheets["表演规则"], { defval: "" }) : [];
    const outcomeRows = workbook.Sheets["发挥档位"] ? XLSX.utils.sheet_to_json(workbook.Sheets["发挥档位"], { defval: "" }) : [];
    const eventRows = workbook.Sheets["现场事件"] ? XLSX.utils.sheet_to_json(workbook.Sheets["现场事件"], { defval: "" }) : [];
    const categoryRows = workbook.Sheets["种类表演特性"] ? XLSX.utils.sheet_to_json(workbook.Sheets["种类表演特性"], { defval: "" }) : [];
  ...
  function chooseTrialInitialItems() {
    const purple = [...state.items.values()].filter((item) => item.quality === "紫");
    const blue = [...state.items.values()].filter((item) => item.quality === "蓝");
    const candidates = [];
    for (const a of purple) for (const b of blue) if (canPackTogether(a, b, state.config.initial_rows, state.config.initial_cols)) candidates.push([a, b]);
    if (candidates.length) return candidates[Math.floor(Math.random() * candidates.length)];
    return [randomItem(purple), randomItem(blue)].filter(Boolean);
  }
  ...
  function switchMode(mode) {
    ...
    renderAll();
    showToast(mode === "trial" ? "已进入试玩模式：通过表演逐步解锁" : "已进入测试模式：60件道具全部可用", "success");
  }
  ...
  function renderDetail() {
    ...
    if (!instance) {
      els.itemDetail.innerHTML = '<div class="detail-empty">选择仓库或背包中的道具，查看数值与效果。</div>';
      return;
    }
    const item = state.items.get(instance.itemId);
    const quality = state.qualities.get(item.quality);
    ...
    els.itemDetail.innerHTML = `
      ...
      <div class="detail-actions">
        ...
        ${state.mode === "test" ? '<button class="button" id="duplicateButton">复制一件到仓库</button>' : ""}
      </div>`;
    ...
    document.getElementById("duplicateButton")?.addEventListener("click", () => {
      pushHistory();
      state.inventory.push(makeInstance(item.id, instance.level));
      renderAll();
      showToast(`已复制 ${item.name} Lv.${instance.level} 到仓库`, "success");
    });
  ...
  function showPerformanceResult(plan, result, stamina, rewardChoice) {
    ...
    els.performanceResultBreakdown.innerHTML = [
      ["基础背包", formatNumber(result.baseScore)],
    ...
    for (const source of state.placed) {
      ...
      if (rule.type === "count") {
        ...
        const active = rule.clauses.every((entry) => {
          ...
          return (entry.min == null || result >= entry.min) && (entry.max == null || result <= entry.max);
        });
        info.active = active;
        if (active) {
